		...
		<template v-if="viewWidth">
			<movable-area class="area" :style="{ height: areaHeight }" @mouseenter="mouseenter"
				...
				<movable-view v-for="(item, index) in imageList" :key="item.id" class="view" direction="all" :y="item.y"
					:x="item.x" :damping="40" :disabled="disabled" @change="onChange($event, item)"
          ...
          opacity: item.opacity 
            ...
            transform: 'scale(' + item.scale + ')' 
						...
						<image class="pre-image" :src="item.picture" mode="aspectFill" @longpress="changeSortState"></image>
	...
	export default {
				...
				handler(n) {
					if (!this.first && this.changeStatus) {
						...
						for (let i = 0; i < n.length; i++) {
							...
							if (this.imageList.length === i || this.imageList[i].picture !== n[i]) {
								flag = true
				...
				handler(n) {
					if (!this.first && this.changeStatus) {
						...
						for (let i = 0; i < n.length; i++) {
							...
							if (this.imageList.length === i || this.imageList[i].picture !== n[i].picture) {
								flag = true
								this.imageList.splice(i)
								this.addProperties(n[i])
							}
		...
		methods: {
			...
			touchend(item) {
				...
				console.log('touchendGoodId', item.id)
				console.log('moveIndex', (item.absY * 3 + item.absX))
				this.$emit('sortGoodsItem', item)
				this.disabled = true
			...
			sortList() {
				...
				for (let s of list) {
					let item = source.find(d => d.picture == s.picture)
					// if (item) {
						result.push(item)
					// } else {
					// 	if (this.keyName !== null) {
				...
				this.$emit('changeSort', sortResult)
			...
			addProperties(item) {
				console.log('addProperties', item);
