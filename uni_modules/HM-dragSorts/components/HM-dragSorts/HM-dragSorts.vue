<template>
	<view class="HM-drag-sort" :style="{'height': ListHeight+'px'}">
		<view class="rowBox-shadow" :style="{'height': rowHeight + 80+'px'}" id="shadowRowBox">
			<view class="row modules" id="shadowRow">
					<!-- 内容  -->
			...
			<block v-for="(tmplist,listType) in dragList" :key="listType">
				<view class="list color" :class="[listType=='A'?(listSwitch?'show':'hide'):(listSwitch?'hide':'show')]">
					<block v-for="(row,index) in tmplist" :key="row.id">
						<u-swipe-action>
							<u-swipe-action-item :options="funcBtn">
								<view :class="'modules row row'+listType" :style="{'height': rowHeight+80+'px'}"
									:data-id="row.id" :id="'row'+listType+row.id" @tap="triggerClick(index,row)">
									<!-- 内容  -->
									<slot name="rowContent" :row="row"></slot>
									<!-- 拖拽图标 -->
									<view class="func-btn" :data-id="row.id"
										:data-type="listType" @touchstart="drag.touchstart" @touchmove="drag.touchmove"
										@touchend="drag.touchend" @longpress="drag.longpress">
										<image :src="imgUrl + 'deliveryIcon.png'" mode=""></image>
									</view>
								</view>
							</u-swipe-action-item>
