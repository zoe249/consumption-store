	...
	<view>
		<view class="add-element">
			<view class="element-name">
				...
				<!-- <view @click="openSelectElement">原料选择</view> -->
			</view>
			<view class="add-box">
				<view>供货商</view>
				<view @click="goSelectSupplier">
					<block v-if="supplier === ''">添加</block>
					<block v-else>{{supplier}}</block>
				</view>
			</view>
			<view class="add-box">
				<view>价格</view>
		...
		<view class="save-btn" @click="saveElement" v-if="!editSupplier">保存</view>
		<view class="supplier-btn" v-if="editSupplier">
			<view @click="deleteElementSupplier">删除</view>
			...
			getElementList() {
				...
				that.request({
					...
					data: {
						storeId: that.storeId,
						name: that.elementName,
						supplierName: that.supplier
					}
				}).then(res => {
					console.log(res)
					const data = res.data.data
					that.selectElementShow = true
					if (data.length != 0) {
