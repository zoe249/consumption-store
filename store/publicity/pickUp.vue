<template>
	<view>
		...
		<view class="banner-pic" v-if="reservationPicture !== ''">
			<view class="delete-img">
				<image :src="imgUrl + 'delete.png'" mode="" @click="selectImg(index)"></image>
			</view>
			<image :src="reservationPicture" mode="widthFix"></image>
		</view>
		...
		methods: {
			...
			editStoreBanner() {
				...
				that.request({
					...
					data: {
						reservationPicture: that.reservationPicture
					}
				}).then(res => {
					console.log('保存预定自提图', res)
			...
			getStorePublicity() {
				...
				that.request({
					...
					console.log('获取预定自提图', res)
					if (res.data.code == '200000') {
						const data = res.data.data
						that.reservationPicture = data.reservationPicture == null ? '' : data.reservationPicture
					} else {
						uni.showToast({
							title: res.data.detail,
							icon: 'none'
