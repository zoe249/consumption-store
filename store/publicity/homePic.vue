<template>
	<view>
		<view class="upload-tip">
			...
			<view>2、建议上传宣传图不超过5张</view>
		</view>
		<view v-for="(item, index) in bannerList" :key="index" class="banner-pic">
		...
		<view class="upload-box">
			<u-upload @afterRead="afterRead" :multiple="true">
		...
		methods: {
			...
			editStoreBanner() {
				...
				that.request({
					...
					data: {
						homePictures: that.bannerList
					}
				}).then(res => {
					console.log('保存首页宣传图', res)
			...
			getStorePublicity() {
				...
				that.request({
					...
					if (res.data.code == '200000') {
						...
						that.bannerList = data.homePictures == null ? [] : data.homePictures
					} else {
						uni.showToast({
							title: res.data.detail,
							icon: 'none'
