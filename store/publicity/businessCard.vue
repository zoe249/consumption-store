<template>
	<view>
		...
		<view class="banner-pic" v-if="businessCard !== ''">
			<view class="delete-img">
				<image :src="imgUrl + 'delete.png'" mode="" @click="selectImg(index)"></image>
			</view>
			<image :src="businessCard" mode="widthFix"></image>
		</view>
		...
		methods: {
			...
			editStoreBanner() {
				...
				that.request({
					...
					data: {
						businessCard: that.businessCard
					}
				}).then(res => {
					console.log('保存卡片背景图', res)
			...
			getStorePublicity() {
				...
				that.request({
					...
					console.log('获取卡片背景图', res)
					if (res.data.code == '200000') {
						const data = res.data.data
						that.businessCard = data.businessCard == null ? '' : data.businessCard
					} else {
						uni.showToast({
							title: res.data.detail,
							icon: 'none'
