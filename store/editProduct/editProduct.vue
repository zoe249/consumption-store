	...
	<view>
		<view class="add-goods-box">
			...
			<view class="upload-cont">
				...
				<block v-else>
					<view class="goods-img-box">
						<image :src="goodsPath" class="goods-img" />
			...
			<!-- <view class="select-element-box">
				<view @click="openElement" class="select-element">原料选择</view>
				<block v-if="selectElementList.length !== 0">
					<view v-for="(item, idx) in selectElementList" :key="idx" class="selected-element">{{item}}</view>
				</block>
			</view> -->
			<view class="element-desc-box" v-if="productClass == '0101'">
				...
				<view class="goods-desc-box" :class="elementDescIdx == 0 ? 'select-element' : ''">
			...
			<view class="bake-time-box">
				<block v-if="bakeType == 1">
					...
					<!-- <view class="bake-time select-bake-time">
						<view class="select-bake-time-img" @click="reduceBakeTime"
							:class="disabledReduceTime ? 'disabled-time' : ''">-</view>
						<input type="digit" v-model="bakeTime" />
						<view class="select-bake-time-img" @click="addBakeTime"
							:class="disabledAddTime ? 'disabled-time' : ''">+</view>
					</view> -->
				</block>
				<block v-if="bakeType == 2">
					...
					<picker mode="time" :value="bakeTimeEnd" @change="changeBakeTimeEnd">
						<view class="bake-time bake-time-end">
							...
							<image src="../../static/goods/bakeTime.png" mode="" class="bake-time-img"></image>
						</view>
					</picker>
		...
		<!-- <view class="bake-hour">
			<view class="bake-hour-tip">烘焙时间（小时）</view>
			<view class="bake-time">
				<u-number-box v-model="bakeTime" :step="0.5" :min="0" :max="24" color="#F2AB99" bgColor="#fff">
				</u-number-box>
			</view>
		</view> -->
		...
		<button class="add-btn" @tap="$u.throttle(verifyGoodsInfo, 500)">保存</button>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.productClass = option.productClass
			if (option.id) {
				this.id = option.id
		...
		methods: {
			...
			getProductsById() {
				...
				that.labelIdx = []
				that.request({
					url: that.config.products + `/${that.id}`,
				}).then(res => {
					let data = res.data.data
					that.key = data.picture.slice(28)
					// 将网络图片下载为临时路径，否则再次上传时会报错
					uni.getImageInfo({
						src: data.picture,
						success: (image) => {
							console.log(image)
					...
					that.marketPrice = data.marketPrice
					that.name = data.name
					that.enName = data.ename === null ? '' : data.ename
					that.label = data.label === null ? '' : data.label
					if (data.label == '0') {
						// 无标签
						that.labelIdx = [0]
					...
					// 出炉时段
					// if (data.bakedEnd == '1')
					// 	that.timeList[0].select = true
					// else if (data.bakedEnd == '2')
					// 	that.timeList[1].select = true
					// else {
					// 	that.timeList[0].select = true
					// 	that.timeList[1].select = true
					// }
					that.fullDescription = data.fullDescription


					// 原料选择或商品特色
					if (data.element !== '' && data.featureDescribe !== '') {
						// 原料和特色都不为空
						that.elementDescIdx = 1
						that.selectElementList = data.element.split(' | ')
						that.element = data.element
						that.featureDescribe = data.featureDescribe == null ? '' : data.featureDescribe
					} else if (data.element !== '') {
						// 原料选择
						that.elementDescIdx = 0
						that.selectElementList = data.element.split(' | ')
						that.element = data.element
					} else if (data.featureDescribe !== '') {
						// 商品特色
						that.elementDescIdx = 1
						that.featureDescribe = data.featureDescribe == null ? '' : data.featureDescribe
					}
					// 烘焙形式
					that.bakeType = data.bakeType
					if (data.bakeType == '1') {
						// 手动烘焙
						if (parseFloat(data.bakeTime) < 1) {
							that.bakeTime = Math.round(data.bakeTime * 60) + '分钟'
						} else {
							that.bakeTime = data.bakeTime + '小时'
						}
					} else if (data.bakeType == '2') {
						// 自动烘焙
						that.bakeTimeEnd = data.bakeTimeEnd
					}
					that.bakeFinishState = data.bakeFinishState
					that.weight = data.weight == null ? '' : data.weight
					that.calorie = data.calorie == null ? '' : data.calorie
				})
			},
			/* 获取七牛云token */
			getQiniuToken() {
			...
			verifyGoodsInfo() {
				...
				// 	// 原料选择
				// 	that.element = that.selectElementList.join(' | ') // 拼接原料
			...
			addProdcut() {
				...
				console.log('保存商品时', that.goodsPath)
				that.request({
					url: that.config.products + `/${that.id}`,
					method: 'PUT',
					header: {
			...
			addElement() {
				...
				that.request({
					...
					if (res.data.code == 200000) {
						...
						that.getElementList()
						// if (!that.selectElement) {
						// 	that.getElementList()
						// } else {
						// 	if (that.elementType == '1') {
						// 		// 主料
						// 		that.mainElementList.push({
						// 			select: false,
						// 			type: that.elementType,
						// 			name: that.elementName
						// 		})
						// 	} else if (that.elementType == '2') {
						// 		// 配料
						// 		that.elementList.push({
						// 			select: false,
						// 			type: that.elementType,
						// 			name: that.elementName
						// 		})
			...
			goEditFullDeac() {
				uni.navigateTo({
					url: '../goods/detail?fullDesc=' + this.fullDescription + '&editGood=true'
				})
			},
			/* 选择标签 */
			selectLabel(res) {
			...
			/* 切换烘焙形式 */
			changeBakeTime(item) {
				if (this.bakeFinishState) {
					uni.showToast({
						title: '该商品正在烘烤，不可修改烘焙形式',
						icon: 'none'
					})
				} else {
			...
			getProductImg() {
				...
				console.log('商品图七牛云地址', 'https://qiaokun.yuepong.com/' + that.key)
